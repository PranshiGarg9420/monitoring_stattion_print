...
#define IO_KEY       "aio_QvbT42zFelLbOcfLJAeC2ZQnZ46T"
...
Adafruit_MQTT_Subscribe green= Adafruit_MQTT_Subscribe(&mqtt, IO_USERNAME"/feeds/Green");
Adafruit_MQTT_Subscribe blue= Adafruit_MQTT_Subscribe(&mqtt, IO_USERNAME"/feeds/Blue");
 ...
 */
Adafruit_MQTT_Publish tc= Adafruit_MQTT_Publish(&mqtt, IO_USERNAME"/feeds/Temperature_C");
Adafruit_MQTT_Publish tf= Adafruit_MQTT_Publish(&mqtt, IO_USERNAME"/feeds/Temperature_F");
Adafruit_MQTT_Publish tk= Adafruit_MQTT_Publish(&mqtt, IO_USERNAME"/feeds/Temperature_K");
...
void loop()
{
  ...
  if(!tc.publish(tempc)|| !tf.publish(tempf)|| !tk.publish(tempk)|| !h.publish(humidity)){
